...
/*********************
 * WeightedGraph class for use in building a weighted maze
 *
...
public class WeightedGraph<T> implements WeightedGraphADT<T>{
   ...
   ********************/    
    public boolean addEdge(T beg, T end, double weight){
        ...
        Edge<T> edge = new Edge(beg1, end1, weight);
        edgeList.add(edge);
		return true;
    }
   ...
   *******************/
    public double getEdgeWeight(T beg, T end){
        ...
        for (int i = 0; i < edgeList.size(); i++){
            if (edgeList.get(i).getOne().getLabel().equals(beg) && edgeList.get(i).getTwo().getLabel().equals(end)) {
                return edgeList.get(i).getWeight();
        }
    }return 0.0; 
}
